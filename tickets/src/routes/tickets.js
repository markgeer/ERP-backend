...
async function ticketsRoutes(fastify, options) {
  ...
  fastify.get('/tickets', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    try {
      ...
      return reply.send({ statusCode: 200, intOpCode: 'SxUS200', data });
    } catch (error) {
      console.error(error);
      return reply.code(500).send({ statusCode: 500, intOpCode: 'ERR500', data: { error: 'Error interno del servidor' } });
    }
  });
  ...
  fastify.get('/tickets/:id', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    try {
      ...
      if (error || !data) {
        return reply.code(404).send({ statusCode: 404, intOpCode: 'ERR404', data: { error: 'Ticket no encontrado' } });
      }
  ...
  fastify.post('/tickets', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    if (!grupo_id || !titulo) {
      return reply.code(400).send({ statusCode: 400, intOpCode: 'ERR400', data: { error: 'grupo_id y titulo son requeridos' } });
    }

    try {
      const { data: estadoPendiente } = await supabase
        .from('estados').select('id').eq('nombre', 'Pendiente').single();

      let prioridadFinal = prioridad_id;
      if (!prioridadFinal) {
        const { data: prioridadMedia } = await supabase
          .from('prioridades').select('id').eq('nombre', 'Media').single();
      ...
      return reply.code(201).send({ statusCode: 201, intOpCode: 'SxUS201', data: nuevoTicket });
    } catch (error) {
      console.error(error);
      return reply.code(500).send({ statusCode: 500, intOpCode: 'ERR500', data: { error: 'Error interno del servidor' } });
    }
  ...
  fastify.put('/tickets/:id', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    try {
      const { data: ticketActual, error: findError } = await supabase
        .from('tickets').select('autor_id').eq('id', id).single();
      ...
      if (Object.keys(updateData).length === 0) {
        return reply.code(400).send({ statusCode: 400, intOpCode: 'ERR400', data: { error: 'No hay campos para actualizar' } });
      }
      ...
      await supabase.from('historial_tickets').insert({
        ...
        accion: 'Ticket actualizado', detalles: updateData
      });
  ...
  // ✅ PATCH /tickets/:id/status - CORREGIDO
  fastify.patch('/tickets/:id/status', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    if (!estado_nombre) {
      return reply.code(400).send({ statusCode: 400, intOpCode: 'ERR400', data: { error: 'estado_nombre es requerido' } });
    }

    try {
      // ✅ Ahora sí traemos grupo_id
      const { data: ticket, error: findError } = await supabase
        .from('tickets')
        .select('id, grupo_id')   // ← sin restricción de autor/asignado
        .eq('id', id)
        .single();
      ...
      const { data: nuevoEstado, error: estadoError } = await supabase
        .from('estados').select('id').eq('nombre', estado_nombre).single();

      if (estadoError || !nuevoEstado) {
        return reply.code(400).send({ statusCode: 400, intOpCode: 'ERR400', data: { error: 'Estado no válido' } });
      }
  ...
  fastify.delete('/tickets/:id', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    try {
      const { data: ticket, error: findError } = await supabase
        .from('tickets').select('id').eq('id', id).single();
      ...
      return reply.send({ statusCode: 200, intOpCode: 'SxUS200', data: { message: 'Ticket eliminado correctamente' } });
    } catch (error) {
      console.error(error);
      return reply.code(500).send({ statusCode: 500, intOpCode: 'ERR500', data: { error: 'Error interno del servidor' } });
    }
  ...
  fastify.get('/tickets/:id/comments', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    try {
      const { data, error } = await supabase
        .from('comentarios')
        .select(`*, autor:autor_id (id, nombre_completo, username)`)
        .eq('ticket_id', id)
  ...
  fastify.post('/tickets/:id/comments', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    if (!contenido) {
      return reply.code(400).send({ statusCode: 400, intOpCode: 'ERR400', data: { error: 'El comentario no puede estar vacío' } });
    }
    ...
    try {
      ...
      return reply.code(201).send({ statusCode: 201, intOpCode: 'SxUS201', data });
    } catch (error) {
      console.error(error);
      return reply.code(500).send({ statusCode: 500, intOpCode: 'ERR500', data: { error: 'Error interno del servidor' } });
    }
  ...
  fastify.get('/tickets/:id/history', { preHandler: [require('../middleware/auth').authMiddleware] }, async (request, reply) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(`*, usuario:usuario_id (id, nombre_completo, username)`)
        .eq('ticket_id', id)
